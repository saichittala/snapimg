class ImageCompressor extends ImageProcessor {
    constructor() {
        super();
        ...
        this.processedFiles = [];
        this.quality = 0.5; // Compression quality (0.1 to 1)
        this.bindEvents();
    }
    ...
    async processImages() {
        try {
            ...
            for (let file of this.files) {
                ...
                try {
                    const processedBlob = await this.compressImage(file, this.quality);
    ...
    async compressImage(file, quality = 0.7) {
        return new Promise((resolve, reject) => {
            ...
            img.onload = () => {
                ...
                // Resize if needed (Example: Max 1920x1080)
                const maxWidth = 1920;
                ...
                // Choose output format (Convert PNG to JPEG for better compression)
    ...
    downloadProcessedFiles() {
        ...
        if (this.processedFiles.length === 1) {
            ...
            zip.generateAsync({ type: 'blob' }).then(content => {
                ...
                this.triggerDownload(url, 'compressed-images.zip');
            }).catch(error => {
    ...
    getOutputName(originalName) {
        return originalName.replace(/\.[^/.]+$/, '') + '_compressed.png';
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.imageProcessor = new ImageCompressor();
});
