class ImageCompressor extends ImageProcessor {
    constructor() {
        ...
        this.qualitySlider = document.getElementById('quality-range'); 
        this.qualityValue = document.getElementById('quality-value'); 
    
        this.processedFiles = [];
        this.quality = this.qualitySlider.value / 100; // Convert to decimal
    ...
    downloadProcessedFiles() {
        ...
        if (this.processedFiles.length === 1) {
            ...
            zip.generateAsync({ type: 'blob' }).then(content => {
                ...
                this.triggerDownload(url, 'snapimg_compressed-images.zip');
            }).catch(error => {
    ...
    getOutputName(originalName) {
        return originalName.replace(/\.[^/.]+$/, '') + '_compressed.jpg';
    }
