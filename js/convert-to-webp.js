class WEBPConverter extends ImageProcessor {
    constructor() {
        super();
        ...
        try {
            ...
            for (let file of this.files) {
                if (!supportedTypes.includes(file.type)) {
                    alert(`Unsupported file type: ${file.name}. Only PNG, JPEG, GIF, and WEBP allowed.`);
                    continue;
                }
                try {
    ...
    getOutputName(originalName) {
        return originalName.replace(/\.[^/.]+$/, '') + '_converted.webp';
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.imageProcessor = new WEBPConverter();
});