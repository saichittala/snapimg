class PNGConverter extends ImageProcessor {
    constructor() {
        super();
    ...
    downloadProcessedFiles() {
        ...
        if (this.processedFiles.length === 1) {
            ...
            zip.generateAsync({ type: 'blob' }).then(content => {
                ...
                this.triggerDownload(url, 'processed-images.zip');
            }).catch(error => {
    ...
    getOutputName(originalName) {
        return originalName.replace(/\.[^/.]+$/, '') + '_converted.png';
    }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    window.imageProcessor = new PNGConverter();
  });